/* Spending Dashboard (Bucketized Categories, Dependent Merchant Filter, No Apply Button)
 * Upload CSV -> parse -> normalize -> filter -> KPIs -> charts + table
 */

// ======= CONFIGURE YOUR CSV LAYOUT HERE =======
const CONFIG = {
    columns: {
        date: "Date", desc: "Description", amount: "Amount", category: "Category", // set "" if not present
    },
    ignoreRows: 0,
    flipSigns: false,
    absAmounts: true,
    ...
    // Coarse category buckets: tweak or add rules as needed (order matters; first match wins)
    bucketRules: [{label: "Groceries", pattern: /(grocery|grocer|supermarket|whole\s*foods|trader\s*joe|market)/i}, {
        label: "Restaurants",
        pattern: /(restaurant|bar|cafe|coffee|fast\s*food|diner|pizza|wings|chicken|starbucks|chipotle|grubhub|ubereats|ubere?)/i
    }, {
        label: "Shopping/Retail",
        pattern: /(retail|shopping|store|department|target|walmart|shein|uniqlo|h&m|clothing|fashion|apparel|jewelry|fragrance)/i
    }, {label: "Subscriptions", pattern: /(subscription|spotify|netflix|prime|membership|service\s*fee)/i}, {
        label: "Transport/Fuel",
        pattern: /(fuel|gas|petrol|shell|exxon|citgo|transport|taxi|rideshare|lyft|uber|metro|train|amtrak|nouria)/i
    }, {
        label: "Travel",
    ...
    }, {label: "Services", pattern: /(services?|barber|repair|vip|business\s*services)/i}, {
        label: "Utilities",
        pattern: /(utility|electric|water|internet|phone|mobile)/i
    }, {label: "Other", pattern: /.*/i}, // catch-all
    ],
};
// =============================================
...
let TOP_N = 10; // from the Top Merchants dropdown
...
function cleanDescription(desc) {
    ...
    s = s.replace(/^(?:apple\s*pay|aplpay|aplp?ay|google\s*pay|venmo|paypal|pp\*|square\*)\s*/i, "");
    s = s.replace(/^(?:bt\*|tst\*|olo\*|sq\*)\s*/i, "");
    s = s.replace(/\s+[-,]?\s*[A-Za-z .&'()]+,\s*[A-Z]{2}\s*$/i, "");
    s = s.replace(/\s+[-,]?\s*[A-Za-z .&'()]+\s+[A-Z]{2}\s*$/i, "");
    s = s.replace(/\s+(?:#|No\.?)?\s*\d{2,5}\s*$/i, "");
    s = s.replace(/\*/g, " ").replace(/[?]+/g, "").replace(/\s{2,}/g, " ").trim();

    const mapRules = [[/amazon\s*marke?t?place.*?/i, "Amazon"], [/amazon\.?com/i, "Amazon"], [/amzn\s*mkp/i, "Amazon"], [/cvs\/?pharmacy/i, "CVS"], [/cvs\s*pharmacy/i, "CVS"], [/walgreens/i, "Walgreens"], [/starbucks/i, "Starbucks"], [/chipotle/i, "Chipotle"], [/grubhub/i, "Grubhub"], [/uber\s*(?:eats)?/i, "Uber"], [/lyft/i, "Lyft"], [/shell/i, "Shell"], [/exxon(?:mobil)?/i, "ExxonMobil"], [/whole\s*foods/i, "Whole Foods"], [/trader\s*joe'?s/i, "Trader Joe's"], [/uniqlo/i, "Uniqlo"], [/sweet\s*green|sweetgreen/i, "Sweetgreen"], [/openai/i, "OpenAI"], [/shein/i, "SHEIN"],];
    for (const [pat, to] of mapRules) s = s.replace(pat, to);

    const preserve = new Set(["CVS", "Uber", "Lyft", "Shell", "ExxonMobil", "Whole Foods", "Trader Joe's", "Uniqlo", "Sweetgreen", "OpenAI", "Amazon", "Starbucks", "Chipotle", "Grubhub", "Target", "SHEIN"]);
    s = s.toLowerCase().replace(/\b\w/g, c => c.toUpperCase());
    for (const brand of preserve) {
        const re = new RegExp("\\b" + brand.replace(/[-/\\^$*+?.()|[\]{}]/g, "\\$&") + "\\b", "gi");
        s = s.replace(re, brand);
    }
    if (s.length <= 2 || /^the$/i.test(s)) s = String(desc).replace(/\*/g, " ").replace(/\s+/g, " ").trim();
...
function bucketizeCategory(raw) {
    ...
    for (const rule of CONFIG.bucketRules) {
        if (rule.pattern.test(s)) return rule.label;
    }
    return "Other";
}
...
function setKPIs(rows) {
    ...
    const merchantSpend = {};
    for (const r of rows) merchantSpend[r.description] = (merchantSpend[r.description] || 0) + r.amount;
...
function buildCategoryData(rows) {
    ...
    let total = 0;
    for (const r of rows) {
        const key = r.bucket; // use bucketized category
        const v = (byCat.get(key) || 0) + r.amount;
        byCat.set(key, v);
        total += r.amount;
    }
    let entries = Array.from(byCat.entries()).sort((a, b) => b[1] - a[1]);

    const minShare = Math.max(0, Math.min(1, CONFIG.categoryPie?.minShare ?? 0.05));
    const maxSlices = Math.max(3, CONFIG.categoryPie?.maxSlices ?? 8);
    const otherLabel = CONFIG.categoryPie?.otherLabel ?? "Other";
...
function buildMerchantData(rows, topN = 10) {
    const byM = new Map();
...
// Rendering
function adjustMerchantChartHeight(numBars) {
    const base = 120, perBar = 28, maxH = 1200, minH = 160;
    const h = Math.max(minH, Math.min(maxH, Math.round(base + perBar * numBars)));
...
function renderCharts(rows) {
    ...
    // Merchant data based on category & merchant filters already applied to 'rows'
...
function renderTable(rows) {
  ...
  `).join("");
    TABLE = new DataTable('#txTable', {
        paging: true, pageLength: 25, lengthMenu: [25, 50, 100], order: [[3, 'desc']], // sort by Amount column (index 3) descending
...
function populateMerchantFilter(rows, selectedBucket) {
    let pool = rows;
...
function applyFilters() {
    ...
    // Merchant filter options depend on bucket selection; refresh options before filtering
    populateMerchantFilter(ROWS, bucket);
...
function hydrateUI(rows) {
    ...
    populateMerchantFilter(ROWS, $("#categoryFilter").value || "");
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    if (merchSel) merchSel.addEventListener("change", applyFilters);
